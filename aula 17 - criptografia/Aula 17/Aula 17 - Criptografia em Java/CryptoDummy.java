...
{  private     byte[]   textoCifrado;
   ...
   {  // Gera uma chave Dummy simetrica:
      int   dk = (int) (Math.random()*(-987654321));
   ...
   {  ObjectInputStream ois = new ObjectInputStream (new FileInputStream (fDummy));  
      ...
      {  textoCifrado[i] = (byte) (textoCifrado[i] + iDummy*505/10);
      }
   }
   ...
   {  ObjectInputStream ois = new ObjectInputStream (new FileInputStream (fDummy));  
      ...
      for(int i = 0; i < texto.length; i++)
      {  textoDecifrado[i] = (byte) (textoDecifrado[i] - iDummy*505/10);
      }
   }
